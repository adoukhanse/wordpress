...
For example: fish, watermelon, cabbage, etc.

= How To Use =
1. To define products which are sold by units but priced by weight go the the edit product screen
...
= How It Works =
1. The customer choose how many units from the item he needs
1. On order creation the item quantity is recalculated based on its weight
1. The shop owner will be able to modify the quantity of it from the edit order screen after he weight the item
...
* **Shop Page Quantity**: Allow to choose product quantity on archive pages, auto-update the cart on quantity change.
...
Salmon (sold by units but priced by weight): Price (General tab)- 45$ (per unit), Weight (Shipping tab)- 3kg (per unit), Quantity Units (Unit Price tab)- 'weight'.
Tomatoes (sold by weight- kg): Price (General tab)- 2$ (per kg), Quantity Step (Unit Price tab)- 0.1, Quantity Suffix- kg.
Nuts (sold by weight- gram): Price (General tab)- 0.03$ (per gram), Quantity Step (Unit Price tab)- 100, Quantity Suffix- gram, Price Quantity- 100, Price Suffix- /100g.
