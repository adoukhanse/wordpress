	...
	class WC_UPW {
		...
		public function wc_add_plugin_links( $links_array, $plugin_file_name, $plugin_data, $status ) {
			if ( strpos( $plugin_file_name, basename( __FILE__ ) ) ) {
				...
				$links_array[] = "<a href=https://$sub_domain.condless.com/contact/>" . __( 'Contact' ) . '</a>';
			}
			return $links_array;
		}
		...
		public function wc_upw_product_tab_icon() {
			echo '<style>#woocommerce-product-data ul.wc-tabs li.unit_price_options a:before { font-family: WooCommerce; content: "\e006"; }</style>';
		}
		...
		public function wc_get_settings() {
			$settings = [
				...
				'product_price_suffix'	=> [
					...
					'desc'		=> __( 'Display the price per unit for simple products which are sold by units but priced by weight', 'unit-price-for-woocommerce' ),
					'type'		=> 'checkbox',
					'default'	=> 'no',
		...
		public function wc_product_unit_price_panel() {
			...
			if ( 'yes' === get_option( 'wc_upw_product_measurement', 'yes' ) ) {
				...
				woocommerce_wp_select( [
					'id'		=> '_upw_measurement',
					'label'		=> __( 'Quantity Units', 'woocommerce' ),
					...
					'options'	=> [
						...
						'mpc'		=> __( 'Measurement Price Calculator', 'woocommerce' ),
					],
				] );
				echo '</div>';
		...
		public function wc_save_product_measurment_options( $product ){
			if ( isset( $_POST['_upw_measurement'] ) ) {
				update_post_meta( $product, '_upw_measurement', wc_clean( $_POST['_upw_measurement'] ) );
			}
			update_post_meta( $product, '_upw_step', 0 < $_POST['_upw_step'] ? wc_clean( $_POST['_upw_step'] ) : '' );
			update_post_meta( $product, '_upw_quantity_suffix', wp_kses_post( $_POST['_upw_quantity_suffix'] ) );
			update_post_meta( $product, '_upw_price_quantity', 0 < $_POST['_upw_price_quantity'] ? wc_clean( $_POST['_upw_price_quantity'] ) : '' );
			update_post_meta( $product, '_upw_price_suffix', wp_kses_post( $_POST['_upw_price_suffix'] ) );
		}
		...
		public function wc_save_variation_measurment_options( $variation_id, $loop ) {
			if ( isset( $_POST["_upw_measurement_$loop"] ) ) {
				update_post_meta( $variation_id, '_upw_measurement', wc_clean( $_POST["_upw_measurement_$loop"] ) );
			}
			update_post_meta( $variation_id, '_upw_step', 0 < $_POST["_upw_step_$loop"] ? wc_clean( $_POST["_upw_step_$loop"] ) : '' );
			update_post_meta( $variation_id, '_upw_quantity_suffix', wp_kses_post( $_POST["_upw_quantity_suffix_$loop"] ) );
			update_post_meta( $variation_id, '_upw_price_quantity', 0 < $_POST["_upw_price_quantity_$loop"] ? wc_clean( $_POST["_upw_price_quantity_$loop"] ) : '' );
			update_post_meta( $variation_id, '_upw_price_suffix', wp_kses_post( $_POST["_upw_price_suffix_$loop"] ) );
		}
		...
		public function wc_quantity_input_args( $args, $product ) {
			...
			if ( $step ) {
				...
				if ( 'quantity' === $args['input_name'] ) {
					...
					} elseif ( 'yes' === get_option( 'wc_upw_quantity_simple' ) && 0 != fmod( 1, $step ) && ( is_shop() || is_product_category() ) ) {
		...
		public function wc_add_custom_data_to_order( $item, $cart_item_key, $values, $order ) {
			...
			switch ( $measurement ) {
				case 'weight':
					...
					break;
				case 'dimensions':
					$unit_size = $product->get_length() * $product->get_width() * $product->get_height();
					$unit_symbol = get_option( 'woocommerce_dimension_unit' );
					switch ( $unit_symbol ) {
						case 'm': $unit_symbol = __( 'm', 'woocommerce' ); break;
						case 'cm': $unit_symbol = __( 'cm', 'woocommerce' ); break;
						case 'mm': $unit_symbol = __( 'mm', 'woocommerce' ); break;
						case 'in': $unit_symbol = __( 'in', 'woocommerce' ); break;
						case 'yd': $unit_symbol = __( 'yd', 'woocommerce' ); break;
					}
					break;
				case 'length':
					$unit_size = $product->get_length();
					$unit_symbol = get_option( 'woocommerce_dimension_unit' );
					break;
				case 'width':
					$unit_size = $product->get_width();
					$unit_symbol = get_option( 'woocommerce_dimension_unit' );
					break;
				case 'height':
					$unit_size = $product->get_height();
					$unit_symbol = get_option( 'woocommerce_dimension_unit' );
					break;
				case 'mpc':
		...
		public function wc_add_header_on_order_item_view( $order ) {
			if ( ! wp_doing_ajax() && $order->get_meta( '_has_unit_items' ) ) { ?>
				<th class="quantity sortable" data-sort="string-ins" style="color:purple"><?php echo __( 'Unit', 'unit-price-for-woocommerce' ) . ' (' . __( 'expected', 'unit-price-for-woocommerce' ) . ')'; ?></th>
				<th class="quantity sortable" data-sort="string-ins" style="color:purple"><?php echo __( 'Unit Cost', 'unit-price-for-woocommerce' ) . ' (' . __( 'expected', 'unit-price-for-woocommerce' ) . ')'; ?></th>
				<th class="quantity sortable" data-sort="string-ins" style="color:purple"><?php echo __( 'Amount of units', 'unit-price-for-woocommerce' ); ?></th>
				<th class="quantity sortable" data-sort="string-ins" style="color:purple"><?php echo __( 'Total', 'woocommerce' ). ' (' . __( 'expected', 'unit-price-for-woocommerce' ) . ')'; ?></th>
			<?php }
		}
		...
		public function wc_change_product_html( $html, $product ) {
			$suffix = '';
			$measurement = $product->get_meta( '_upw_measurement' );
			if ( $product->has_weight() && $product->is_type( 'simple' ) ) {
				switch ( $measurement ) {
					case 'weight':
						$unit_symbol = get_option( 'woocommerce_weight_unit' );
						switch ( $unit_symbol ) {
						...
						$suffix = '<br><small>' . wc_price( wc_get_price_to_display( $product, [ 'qty' => 1 / $product->get_weight() ] ) ) . '/' . $unit_symbol . '</small>';
						break;
				}
			}
		...
		/**
		 * Set the quantity of cart item based on selected value
		 * @return mixed
		 */
		...
		public function wc_set_item_quantity_js() { ?>
			<style>.hidden-button { display: none !important }</style>
			<script type="text/javascript">
			jQuery( function( $ ) {
				var timeout;
		...
		public function wc_variable_set_item_quantity_js() { ?>
			<style>.woocommerce-variation-add-to-cart { display: none !important; }</style>
			<script type="text/javascript">
			jQuery( function( $ ) {
				var timeout;
				$( document.body ).on( 'change', 'input.qty', function() {
		...
		public function wc_archive_quantity_field() {
			if ( function_exists( 'is_plugin_active' ) && ( is_plugin_active( 'qty-increment-buttons-for-woocommerce/qty-increment-buttons-for-woocommerce.php' ) || is_plugin_active( 'wc-quantity-plus-minus-button/init.php' ) ) ) {
				...
				$theme = wp_get_theme();
				$is_ocean = in_array( 'OceanWP', [ $theme->name, $theme->parent_theme ] );
				$base_selector = apply_filters( 'upw_quantity_base_selector', '$( this ).parent().parent()' );
				$base_trigger = apply_filters( 'upw_quantity_base_trigger', $is_ocean ? 'change' : 'click input' );
				$on_trigger = apply_filters( 'upw_quantity_on_trigger', $is_ocean ? 'change' : 'click' );
			} ?>
			<script type="text/javascript">
			jQuery( function( $ ) {
		...
		/**
		 * Add Ajax variation add to cart
		 */
		public function wc_ajax_add_to_cart() {
			...
			if ( $variation_id && $passed_validation && WC()->cart->add_to_cart( $product_id, $quantity, $variation_id ) && 'publish' === get_post_status( $product_id ) ) {
		...
		public function wc_ajax_variation() { ?>
			<style>.archive .variations_form td { background: none; }</style>
			<script type="text/javascript">
			jQuery( function( $ ) {
		...
		public function wc_adjust_price_display( $price_html, $product ) {
			...
			if ( $price_quantity ) {
				$price_html = wc_price( wc_get_price_to_display( $product, [ 'qty' => $price_quantity ] ) );
			}
		...
		/**
		 * Display the price by quantity for simple products on single pages
		 */
		public function wc_product_price_quantity() {
			...
			if ( $product->is_type( 'simple' ) && apply_filters( 'upw_display_product_subtotal', true, $product ) ) {
				$base_selector = function_exists( 'is_plugin_active' ) && ( is_plugin_active( 'qty-increment-buttons-for-woocommerce/qty-increment-buttons-for-woocommerce.php' ) || is_plugin_active( 'wc-quantity-plus-minus-button/init.php' ) ) ? '$( this ).parent().parent().parent()' : '$( this ).parent().parent()';
		...
		public function wc_archive_product_price_quantity() {
			global $product;
			if ( ( is_shop() || is_product_category() ) && $product->is_type( 'simple' ) && $product->is_purchasable() && apply_filters( 'upw_display_product_subtotal', true, $product ) && apply_filters( 'upw_simple_subtotal_enabled', 'yes' === get_option( 'wc_upw_quantity_simple' ) || function_exists( 'is_plugin_active' ) && is_plugin_active( 'qty-increment-buttons-for-woocommerce/qty-increment-buttons-for-woocommerce.php' ) ) ) {
