...
const Style = ({settings, breakpoints, cssHelper})=>{
    ...
    cssHelper.add('.shopengine-shopengine-filterable-product-list .filter-content-row', shopengine_products_column_no, (val) => {
        return `
    ...
    cssHelper.add('.filter-nav li:not(:last-child)::before',{}, (val) => {
        return `
        content: ""; 
        position: absolute; 
        top: 0; 
    ...
    cssHelper.add('.shopengine-shopengine-filterable-product-list .filter-content-row',settings.shopengine_product_wrap_gap, (val) => {
        return `
        grid-gap: ${val}px;
        `
    } )
