<div class="shopengine shopengine-widget">
	...
	?>
	<div class="shopengine-<?php echo esc_attr($block->block_key); ?>" data-widget_settings='<?php echo json_encode($is_content); ?>'>
    ...
    ------------------------- -->
			<div class="filter-nav">
				<ul>
					<?php if (!empty($settings['shopengine_filter_content'])) :
						foreach ($settings['shopengine_filter_content'] as $key => $content) :

							// collect navbar label
							array_push($uid, uniqid());
							if ($key == 0) {
								$products_lists = $content['product_list']['desktop'];
								$products = wp_list_pluck($products_lists, 'id');
							}
					...
					?>		
							...
							<li class="filter-nav-item">
								<a href="#" class="filter-nav-link <?php echo esc_attr($key == 0 ? 'active' : ''); ?>" data-filter-uid="<?php echo esc_attr($uid[$count]); ?>" data-product-list='<?php echo !empty($all_product_list) ? json_encode($all_product_list) : ''; ?>'>
